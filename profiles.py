...
WORKSPACE_ROOT = Path("/mnt/c/Users/Administrator/codex")
...
def validate_payload(source_block: str, payload: dict[str, Any]) -> list[str]:
    ...
    for index, item in enumerate(payload[profile.result_list_field]):
        ...
        evidence_page = str(item.get("evidence_page", "") or "").strip()
        if evidence_snippet == "":
            errors.append(f"{profile.result_list_field}[{index}] missing evidence_snippet")
        if evidence_page == "":
            errors.append(f"{profile.result_list_field}[{index}] missing evidence_page")
